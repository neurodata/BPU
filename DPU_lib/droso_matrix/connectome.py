...
import numpy as np
import os
from DPU_lib.basics import get_project_base_dir
from DPU_lib.droso_matrix.utils import normalize_matrix
...
def load_sio_conn_zaq(connectivity_path, annotation_path, rescale_factor=4e-2, normalization='minmax',
                      input_type='visual',
                      output_type = 'output',
                      use_KC = False):
    ...
    df_category = pd.read_pickle(os.path.join(get_project_base_dir(),annotation_path))
    ...
    df_conn = pd.read_csv(os.path.join(get_project_base_dir(), connectivity_path), index_col=0)
    df_conn.index = df_conn.index.astype(int)
    df_conn.columns = df_conn.columns.astype(int)
    all_neuron_ids = sorted(df_conn.index.tolist())
    print(f"Connectivity matrix contains {len(all_neuron_ids)} neurons")
    ...
    print(f"After filtering, found {len(valid_output_ids)} output neurons in matrix")
    print(f"After filtering, found {len(valid_KC_ids)} KC neurons in matrix")
    print(f"Remaining {len(valid_internal_ids)} neurons classified as internal")

    # Create the SIO-ordered adjacency matrix
    ordered_ids = valid_sensory_ids + valid_KC_ids + valid_internal_ids + valid_output_ids
    df_conn_sio = df_conn.loc[ordered_ids, ordered_ids]
    adjacency = df_conn_sio.values  # shape: [N, N]
    ...
    return {
        ...
        'internal_idx': [ordered_ids.index(i) for i in valid_internal_ids],
